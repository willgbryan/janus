...
forecast_length = 90  # number of periods to forecast ahead
...
prediction_interval = (
    0.75  # sets the upper and lower forecast range by probability range. Bigger = wider
)
...
forecast_csv_name = "ray_request_90_horizon"  # f"autots_forecast_{forecast_name}.csv"  # or None, point forecast only is written
model_list = [
    'Cassandra',
]
...
tickers = ["AAPL", "TSLA", "NVDA", "MSFT", "AMD", "AVGO", "PATH", "GOOG", "CRM", "SPLK", "FVRR", "DIS"]
trend_list = ["forecasting", "aapl", "tsla", "nvda", "msft", "amd", "avgo", "path", "goog", "crm", "splk", "fvrr", "dis"]
...
df = load_live_daily(
    ...
    weather_years=3,
    london_air_days=700,
    wikipedia_pages=['all', 'Apple_Inc', "Tesla,_Inc", "YouTube", "United_States"],
    gsa_key=gsa_key,
